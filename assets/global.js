...
class QuantityInput extends HTMLElement {
  ...
  onInputChange(event) {
    ...
    // Check for empty or invalid input (NaN or <= 0)
  ...
  onButtonClick(event) {
    ...
    if (this.enableSliderLooping) {
      ...
      if (maxScroll > 2) {
        ...
        const nearStart = this.slider.scrollLeft <= 2;
        const nearEnd = this.slider.scrollLeft >= maxScroll - 2;
        ...
        if (!isNext && nearStart) {
          this.slider.scrollTo({ left: maxScroll });
          return;
        }
      }
    }
...
class VariantSelects extends HTMLElement {
  ...
  updateMedia() {
    ...
    mediaGalleries.forEach((mediaGallery) => {
      if (mediaGallery.filterByAlt) {
        mediaGallery.filterByAlt(this.currentVariant.options);
      }
    });
